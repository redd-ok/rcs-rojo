...
return function(Iris: Types.Iris): Types.Internal
	...
	function Internal._cycle(deltaTime: number)
		...
		if
			Internal._rootInstance == nil
			or Internal._rootInstance.Parent == nil
		then
			Iris.ForceRefresh()
		end
		...
		for _, widget: Types.Widget in Internal._lastVDOM do
			if
				widget.lastCycleTick ~= Internal._cycleTick
				and (widget.lastCycleTick ~= -1)
			then
		...
		if Internal._fullErrorTracebacks then
			...
			if coroutineStatus == "suspended" then
				...
				local _, success, result =
					coroutine.resume(Internal._cycleCoroutine)
	...
	function Internal.WidgetConstructor(
		type: string,
	...
	function Internal._Insert(
		widgetType: string,
		args: Types.WidgetArguments?,
		...
		if args ~= nil then
			...
			for index: number, argument: Types.Argument in args do
				assert(
					index > 0,
					`Widget Arguments must be a positive number, not {index} of type {typeof(
						index
					)} for {argument}.`
				)
		...
		local thisWidget: Types.Widget = if lastWidget == nil
		...
		if
			Internal._deepCompare(thisWidget.providedArguments, arguments)
			== false
		then
	...
	function Internal._GenNewWidget(
		...
		if Internal._config.Parent then
			...
			thisWidget.Instance.Parent =
				Internal._widgets[parentWidget.type].ChildAdded(
					parentWidget,
					thisWidget
				)
		end
			...
			if states then
				for index: string, state: Types.State<any> in states do
					if
						not (
							type(state) == "table"
							and getmetatable(state :: any)
								== Internal.StateClass
						)
					then
						-- generate a new state.
						states[index] =
							Internal._widgetState(stateWidget, index, state)
					end
	...
	function Internal._ContinueWidget(
		ID: Types.ID,
		widgetType: string
	): Types.Widget
		local thisWidgetClass: Types.WidgetClass = Internal._widgets[widgetType]
	...
	function Internal._DiscardWidget(widgetToDiscard: Types.Widget)
		...
		if widgetParent then
			...
			Internal._widgets[widgetParent.type].ChildDiscarded(
				widgetParent,
				widgetToDiscard
			)
		end
	...
	function Internal._EventCall(
		...
		assert(
			Event ~= nil,
			`widget {thisWidget.type} has no event of name {eventName}`
		)
	...
	function Internal._generateRootInstance()
		...
		Internal._rootInstance =
			Internal._widgets["Root"].Generate(Internal._widgets["Root"])
	...
	function Internal._generateSelectionImageObject()
		...
		SelectionImageObject.BackgroundColor3 =
			Internal._config.SelectionImageObjectColor
		SelectionImageObject.BackgroundTransparency =
			Internal._config.SelectionImageObjectTransparency
	...
	function Internal._getID(levelsToIgnore: number): Types.ID
		...
		local discriminator = if Internal._pushedId
			then Internal._pushedId
			else Internal._usedIDs[ID]

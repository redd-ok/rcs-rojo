...
function Iris.Init(
	...
	assert(
		...
		"Iris.Init() cannot be called once shutdown."
	)

	if parentInstance == nil then
		-- coalesce to playerGui
		parentInstance = game:GetService("Players").LocalPlayer
...
function Iris:Connect(
	callback: () -> ()
): () -> () -- this uses method syntax for no reason.
...
function Iris.Append(userInstance: GuiObject)
	...
	if Internal._config.Parent then
		...
		widgetInstanceParent = Internal._widgets[parentWidget.type].ChildAdded(
			parentWidget,
			{ type = "userInstance" } :: Types.Widget
		)
	end
...
function Iris.PushId(ID: Types.ID)
	assert(
		typeof(ID) == "string",
		"The ID argument to Iris.PushId() to be a string."
	)
...
function Iris.ComputedState<T, U>(
	firstState: Types.State<T>,

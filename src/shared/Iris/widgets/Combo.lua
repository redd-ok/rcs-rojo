...
return function(Iris: Types.Internal, widgets: Types.WidgetUtility)
	...
	local function UpdateChildContainerTransform(thisWidget: Types.Combo)
		...
		ChildContainer.Size =
			UDim2.fromOffset(PreviewContainer.AbsoluteSize.X, 0)

		local previewPosition: Vector2 = PreviewContainer.AbsolutePosition
			- widgets.GuiOffset
	...
	widgets.registerEvent("InputBegan", function(inputObject: InputObject)
		...
		local rectMin: Vector2 = PreviewContainer.AbsolutePosition
			- widgets.GuiOffset
		local rectMax: Vector2 = PreviewContainer.AbsolutePosition
			- widgets.GuiOffset
			+ PreviewContainer.AbsoluteSize
		if widgets.isPosInsideRect(MouseLocation, rectMin, rectMax) then
			return
		end

		rectMin = ChildContainer.AbsolutePosition - widgets.GuiOffset
		rectMax = ChildContainer.AbsolutePosition
			- widgets.GuiOffset
			+ ChildContainer.AbsoluteSize
		if widgets.isPosInsideRect(MouseLocation, rectMin, rectMax) then
			return
		end

...
return function(Iris: Types.Iris)
	...
	local function wrapper(name: string)
		return function(
			arguments: Types.WidgetArguments?,
	...
	Iris.ComboArray = function<T>(
		...
		local thisWidget =
			Iris.Internal._Insert("Combo", arguments, defaultState)
		local sharedIndex: Types.State<T> = thisWidget.state.index
		for _, Selection in selectionArray do
			Iris.Internal._Insert(
				"Selectable",
				{ Selection, Selection },
				{ index = sharedIndex } :: Types.States
			)
		end
		Iris.End()
	...
	Iris.NextColumn = function()
		...
		assert(
			parentWidget.type == "Table",
			"Iris.NextColumn() can only be called within a table."
		)
	...
	Iris.SetColumnIndex = function(columnIndex: number)
		...
		assert(
			parentWidget.type == "Table",
			"Iris.SetColumnIndex() can only be called within a table."
		)
		assert(
			columnIndex >= parentWidget.InitialNumColumns,
			"Iris.SetColumnIndex() argument must be in column range."
		)
		parentWidget.RowColumnIndex = math.floor(
			parentWidget.RowColumnIndex / parentWidget.InitialNumColumns
		) + (columnIndex - 1)
	end
	...
	Iris.NextRow = function()
		...
		local nextRow: number = math.floor(
			(parentWidget.RowColumnIndex + 1) / InitialNumColumns
		) * InitialNumColumns

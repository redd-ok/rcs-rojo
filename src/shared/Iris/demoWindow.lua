...
return function(Iris: Types.Iris)
	...
	local widgetDemos = {
		...
		Image = function()
			...
			do
				...
				local AssetState =
					Iris.State("rbxasset://textures/ui/common/robux.png")
				...
				local PixelatedState = Iris.ComputedState(
					PixelatedCheckState,
					function(check: boolean)
						return check and Enum.ResamplerMode.Pixelated
							or Enum.ResamplerMode.Default
					end
				)

				local ImageColorState = Iris.State(Iris._config.ImageColor)
				local ImageTransparencyState =
					Iris.State(Iris._config.ImageTransparency)
				Iris.InputColor4({ "Image Tint" }, {
				...
				Iris.SliderUDim2(
					{ "Image Size", nil, nil, UDim2.new(1, 240, 1, 240) },
					{ number = SizeState }
				)
				Iris.SliderRect(
					{ "Image Rect", nil, nil, Rect.new(256, 256, 256, 256) },
					{ number = RectState }
				)
				...
				Iris.SliderUDim2(
					{ "Tile Size", nil, nil, UDim2.new(1, 240, 1, 240) },
					{ number = TileState }
				)
		...
		Selectable = function()
			...
			do
				...
				if
					Iris.Selectable(
						{ "Double click Selectable", 3, true },
						{ index = sharedIndex }
					).doubleClicked()
				then
					sharedIndex:set(3)
		...
		Combo = function()
			...
			do
				...
				do
					...
					if
						NoPreview.checked()
						and NoButton.isChecked.value == true
					then
						NoButton.isChecked:set(false)
					end
					if
						NoButton.checked()
						and NoPreview.isChecked.value == true
					then
				...
				do
					...
					do
						...
						do
							Iris.Text({ "Who loves apples?" })
							if
								Iris.Button({ "I don't like apples." })
									.clicked()
							then
								a:set(false)
							end
						...
						do
							Iris.Text({ "But carrots are the best." })
							if
								Iris.Button({ "I disagree with you." })
									.clicked()
							then
								c:set(false)
							end
					...
					Iris.Separator()
					if
						Iris.Button({ "Actually, let me reconsider it." })
							.clicked()
					then
						a:set(true)
						b:set(true)
		...
		Input = function()
			...
			do
				...
				if
					NoFieldCheckbox.checked()
					and NoButtonsCheckbox.isChecked.value == true
				then
					NoButtonsCheckbox.isChecked:set(false)
				end
				if
					NoButtonsCheckbox.checked()
					and NoFieldCheckbox.isChecked.value == true
				then
				...
				Iris.SliderNum(
					{ "Transparency", 0.01, 0, 1 },
					{ number = transparency }
				)
				Iris.InputColor3(
					{ "InputColor3", UseFloats:get(), UseHSV:get() },
					{ color = sharedColor }
				)
				Iris.InputColor4(
				...
				do
					...
					helpMarker(
						"ctrl + click slider number widgets to input a number"
					)
				end
				Iris.End()
				Iris.PushConfig({ ContentWidth = UDim.new(1, -120) })
		...
		InputText = function()
			...
			do
				local InputText =
					Iris.InputText({ "Input Text Test", "Input Text here" })
		...
		Tooltip = function()
			...
			do
				if
					Iris.Text({ "Hover over me to reveal a tooltip" }).hovered()
				then
				...
				if
					Iris.Checkbox({ "Show dynamic text tooltip" }).state.isChecked.value
				then
		...
		Plotting = function()
			...
			do
				...
				Iris.ProgressBar({
					"Progress Bar",
					`{math.floor(Progress:get() * 1753)}/1753`,
				}, { progress = Progress })
				...
				do
					...
					Iris.PlotHistogram(
						{ "Histogram", 100, 0, 1, "random" },
						{ values = ValueState }
					)
					Iris.PlotLines(
						{ "Lines", 100, 0, 1, "random" },
						{ values = ValueState }
					)
				end
				...
				do
					...
					local samples = Iris.SliderNum(
						{ "Samples", 1, 1, 145, "%d samples" },
						{ number = SampleState }
					)
					if
						Iris.SliderNum(
							{ "Baseline", 0.1, -1, 1 },
							{ number = BaseLineState }
						)
							.numberChanged()
					then
					...
					then
						if Animated.state.isChecked.value then
							TimeState:set(
								TimeState.value + Iris.Internal._deltaTime
							)
						end
						...
						for i = 1, SampleState.value do
							if func == "Sin" then
								ValueState.value[i] =
									math.sin(math.rad(5 * (i + offset)))
							elseif func == "Cos" then
								ValueState.value[i] =
									math.cos(math.rad(5 * (i + offset)))
							elseif func == "Tan" then
								ValueState.value[i] =
									math.tan(math.rad(5 * (i + offset)))
							elseif func == "Saw" then
								ValueState.value[i] = if (i % 2)
										== (offset % 2)
									then 1
									else -1
							end
					...
					Iris.PlotHistogram(
						{ "Histogram", 100, -1, 1, "", BaseLineState:get() },
						{ values = ValueState }
					)
					Iris.PlotLines(
						{ "Lines", 100, -1, 1 },
		...
		do
			...
			do
				...
				elseif enteredState then
					...
					Iris.Text({
						string.format(
							"state has: %d connected functions",
							#enteredState.ConnectedFunctions
						),
					})
					Iris.End()
				else
					Iris.Text({
						string.format(
							'The ID, "%s", is not a state or widget',
							enteredText
						),
					})
				end
	...
	local function debugPanel()
		...
		do
			...
			do
				...
				Iris.Text({
					`MouseOffset: {Iris.Internal._utility.MouseOffset}`,
				})

				Iris.SeparatorText({ "UserInputService" })
				Iris.Text({
					`MousePosition: {Iris.Internal._utility.UserInputService:GetMouseLocation()}`,
				})
				Iris.Text({
					`MouseLocation: {Iris.Internal._utility.getMouseLocation()}`,
				})

				Iris.Text({
					`Left Control: {Iris.Internal._utility.UserInputService:IsKeyDown(
						Enum.KeyCode.LeftControl
					)}`,
				})
				Iris.Text({
					`Right Control: {Iris.Internal._utility.UserInputService:IsKeyDown(
			...
			local styleList = {
				{
					...
					function()
						...
						do
							...
							helpMarker(
								"Update the global config with these changes."
							)
						end
						Iris.End()
						...
						local function SliderInput(
							...
							local Input = Iris[input](arguments, {
								number = Iris.WeakState(
									Iris._config[arguments[1]]
								),
							})
							if Input.numberChanged() then
								UpdatedConfig.value[arguments[1]] =
									Input.number:get()
							end
						end
						...
						SliderInput(
							"SliderNum",
							{ "SeparatorTextBorderSize", 1, 0, 20 }
						)
						...
						SliderInput(
							"SliderNum",
							{ "ImageBorderSize", 1, 0, 12 }
						)
						local TitleInput = Iris.ComboEnum(
							{ "WindowTitleAlign" },
							{
								index = Iris.WeakState(
									Iris._config.WindowTitleAlign
								),
							},
							Enum.LeftRight
						)
						if TitleInput.closed() then
							UpdatedConfig.value["WindowTitleAlign"] =
								TitleInput.index:get()
						end
						...
						SliderInput(
							"SliderNum",
							{ "MouseDoubleClickTime", 0.1, 0, 5 }
						)
						SliderInput(
							"SliderNum",
							{ "MouseDoubleClickMaxDist", 0.1, 0, 20 }
						)
					end,
				},
				{
					...
					function()
						...
						for _, vColor in color4s do
							local Input = Iris.InputColor4({ vColor }, {
								color = Iris.WeakState(
									Iris._config[vColor .. "Color"]
								),
								transparency = Iris.WeakState(
									Iris._config[vColor .. "Transparency"]
								),
							})
							if Input.numberChanged() then
								UpdatedConfig.value[vColor .. "Color"] =
									Input.color:get()
								UpdatedConfig.value[vColor .. "Transparency"] =
									Input.transparency:get()
							end
						end
					end,
				},
				{
					...
					function()
						...
						if TextSize.numberChanged() then
							UpdatedConfig.value["TextSize"] =
								TextSize.state.number:get()
						end

						Iris.SeparatorText({ "Properties" })

						local TextFont =
							Iris.WeakState(Iris._config.TextFont.Family)
						local FontWeight = Iris.ComboEnum({ "Font Weight" }, {
							index = Iris.WeakState(
								Iris._config.TextFont.Weight
							),
						}, Enum.FontWeight)
						local FontStyle = Iris.ComboEnum({ "Font Style" }, {
						...
						for name: string, font: Font in fonts do
							font = Font.new(
								font.Family,
								FontWeight.state.index.value,
								FontStyle.state.index.value
							)
							Iris.SameLine()
							do
								Iris.PushConfig({
	...
	local function widgetEventInteractivity()
		...
		do
			...
			do
				local button =
					Iris.Button({ selectedEvent:get() .. " to reveal text" })
			...
			do
				if
					Iris.Button({ "Click to show text for 20 frames" })
						.clicked()
				then
	...
	local function widgetStateInteractivity()
		...
		do
			...
			local checkboxState3 = Iris.ComputedState(
				checkboxState2,
				function(newValue)
					return not newValue
				end
	...
	local function layoutDemo()
		...
		do
			...
			do
				...
				-- formula to cycle between 0 and 100 linearly
				local newValue = math.clamp(
					(math.abs((os.clock() * 15) % 100 - 50)) - 7.5,
					0,
					35
				) / 35
				progress:set(newValue)
